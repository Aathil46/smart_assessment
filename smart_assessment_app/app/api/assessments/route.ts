...
export async function POST(request: Request) {
  try {
    ...
    if (profile?.role !== "teacher") return NextResponse.json({ error: { code: "FORBIDDEN", message: "Teacher access required." } }, { status: 403 });
    const { data: klass } = await admin.from("classes").select("id").eq("id", payload.classId).eq("teacher_id", auth.user.id).single();
    ...
    const { data, error } = await admin.from("assessments").insert({ id: randomUUID(), class_id: payload.classId, material_id: payload.materialId, title: payload.title, topic: payload.topic, num_questions: payload.numQuestions, difficulty: payload.difficulty, created_by: auth.user.id, status: "draft" }).select("id,status").single();
